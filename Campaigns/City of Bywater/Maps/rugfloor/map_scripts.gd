...
static func Allow_Char_Swap() :
	...
	SfxPlayer.play()
	var textRect = NodeAccess.Get.__UI().ow_hud.textRect
	textRect.set_text("You may swap characters here.", false)
	GameGlobal.allow_character_swap(true)
	MusicStreamPlayer.play_music_type("Outdoor")
...
static func Take_Stairs_U() :
	var textRect = NodeAccess.Get.__UI().ow_hud.textRect
	textRect.set_text("You take the stairs up to default_map !", true)
